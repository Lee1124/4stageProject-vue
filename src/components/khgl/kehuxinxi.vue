  ...
  var productsMethods = {
    ...
    productEdit(index, row) {
      ...
      '    <br>\n' +
        ...
        var id=$("#productForm #productText").val();
        var name=$("#productForm #productPrice").val();
        var sex=$("#productForm #productTp").find("option:selected").text();
        var dizhi=$(".biangeren #zhuzhi").val();
        var birthday=$(" .biangeren #shengri").val();
        var phone=$(".biangeren #phone").val();
        var vip=$(".biangeren #vip").val();
        this.$axios.get("/api/biangeren.do",{params:{id:id,sex:sex,dizhi:dizhi,birthday:birthday,phone:phone,vip:vip,name:name}})
        .then((res) => {
